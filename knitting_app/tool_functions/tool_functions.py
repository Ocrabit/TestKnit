...
def center_window(frame):
    # Force the frame to update its dimensions
    ...
    print(f'width {(window_width // 2)}', 'height', f'{(window_height // 2)}')
    position_x = (screen_width // 2) - (window_width // 2)
    position_y = (screen_height // 2) - (window_height // 2)

    # Set the geometry of the window to center it
    frame.geometry(f'+{position_x}+{position_y}')


def center_on_parent_window(parent, frame):
    # Force the parent and frame to update their dimensions
    frame.update_idletasks()
    parent.update_idletasks()

    # Get the frame's current width and height
    window_width = frame.winfo_width()
    window_height = frame.winfo_height()

    # Get the parent frame's current position and size
    parent_x = parent.winfo_x()
    parent_y = parent.winfo_y()
    parent_width = parent.winfo_width()
    parent_height = parent.winfo_height()

    # Get the screen width and height of the monitor where the parent window is
    screen_width = parent.winfo_screenwidth()
    screen_height = parent.winfo_screenheight()

    # Calculate the position for the child window to be centered relative to the parent
    position_x = parent_x + (parent_width // 2) - (window_width // 2)
    position_y = parent_y + (parent_height // 2) - (window_height // 2)

    # Ensure the window stays within the active monitor bounds
    position_x = max(0, min(position_x, screen_width - window_width))
    position_y = max(0, min(position_y, screen_height - window_height))

    # Set the geometry of the child window to center it relative to the parent
    frame.geometry(f'+{position_x}+{position_y}')
