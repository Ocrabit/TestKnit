...
class Sweater:
    ...
    def save_arrays(self):
        ...
        self.saved_path = Path(__file__).parent.parent / f"data/patterns/saved_patterns/{save_name}"
    ...
    '''def printSweater(self, orientation="down"):
        if self.sleeve and self.torso:
            sleeve_img = self.sleeve.getImage()
            body_img = self.torso.getImage()
            sleeve_w, sleeve_h = sleeve_img.size
            body_w, body_h = body_img.size
            sweater_img = None
            if orientation == "down":
                sweater_w = sleeve_w * 2 + body_w
                sweater_h = max(sleeve_h, body_h)
                sweater_img = Image.new('RGB', (sweater_w, sweater_h), (255, 255, 255))
                sweater_img.paste(sleeve_img, (0, 0))
                sweater_img.paste(body_img, (sleeve_w, 0))
                sweater_img.paste(sleeve_img, (sweater_w - sleeve_w, 0))
            elif orientation == "spread":
                sweater_w = body_w + sleeve_h * 2
                sweater_h = body_h
                sweater_img = Image.new('RGB', (sweater_w, sweater_h), (255, 255, 255))
                sweater_img.paste(sleeve_img.rotate(-90, expand=True), (0, 0))
                sweater_img.paste(body_img, (sleeve_h, 0))
                sweater_img.paste(sleeve_img.rotate(-90, expand=True), (sweater_w - sleeve_h, 0))
            else:
                print("Orientation must be down or spread")
            return sweater_img'''
